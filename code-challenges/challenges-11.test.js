...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArray = input.map(element => element.filter (value => typeof(value) === 'number' && !(value % 5)));
  let result = newArray.map(val => val.map(elem => Math.pow(2,elem)));
  return result;
};
...
let findShortest = (data) => {
  // Solution code here...
  let sorted = data.sort((elemnt1,elemnt2) => elemnt1.height - elemnt2.height );
